...
public class ReplayServiceImpl implements ReplayService {
  ...
  public RepeaterResult<String> replay(ReplayParams params) {
    Optional.ofNullable(params.getModuleId())
        .orElseThrow(() -> new RuntimeException("id can not be null"));
    Optional.ofNullable(params.getAppName())
        .orElseThrow(() -> new RuntimeException("appName can not be null"));
    Optional.ofNullable(params.getTraceId())
        .orElseThrow(() -> new RuntimeException("traceId can not be null"));
    RepeaterResult<ModuleInfoBO> result = moduleInfoService.queryByIdAndAppName(params.getAppName(),
        params.getModuleId());
    ...
    final Record record = recordDao.selectByAppNameAndTraceId(params.getAppName(),
        params.getTraceId());
    if (record == null) {
  ...
  public RepeaterResult<String> saveRepeat(String body) {
    ...
    replay.setStatus(
        rm.isFinish() ? ReplayStatus.FINISH.getStatus() : ReplayStatus.FAILED.getStatus());
    ...
    // simple compare
    if (StringUtils.isNotBlank(replay.getIngoreKeys()) && actual instanceof Map
        && expect instanceof Map) {
  ...
  public PageResult<ReplayListBO> list(ReplayParams params) {
    ...
    if (page.hasContent()) {
      ...
      result.setData(page.getContent().stream().map(replayListConverter::convert)
  ...
  private RepeaterResult<String> doRepeat(Record record, ReplayParams params) {
    ...
    try {
      requestParams.put(Constants.DATA_TRANSPORT_IDENTIFY,
          SerializerWrapper.hessianSerialize(meta));
    } catch (SerializeException e) {
      return RepeaterResult.builder().success(false).message(e.getMessage()).build();
    }
    LogUtil.info("回放地址：{},IP:{},PORT:{}", repeatURL, params.getIp(), params.getPort());
    HttpUtil.Resp resp = HttpUtil.doPost(
        String.format(repeatURL, params.getIp(), params.getPort(), params.getNamespace()),
        requestParams);
    if (resp.isSuccess()) {
      return RepeaterResult.builder().success(true).message("operate success")
          .data(meta.getRepeatId()).build();
    }
